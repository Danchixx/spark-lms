// src/pages/SuperAdmin/Approvals/components/ApprovalUsers.jsx
...
// ── Confirmation result screen (shared) ───────────────────────
...
// ── Reason modal — shown before suspend/ban ───────────────────
const ReasonModal = ({ actionLabel, actionColor, onConfirm, onCancel }) => {
...
// ── Approval modal — for Pending users ────────────────────────
const ApprovalModal = ({ user, company, onClose, onApprove, onReject }) => {
          ...
          <>
            ...
            <div style={{
              ...
              <div style={{
                ...
                <div style={{ width: "100%" }}>
                  ...
                  <div style={{
                    ...
                    border: "none",
                    padding: "8px 10px", fontSize: 13, color: "#333",
              ...
              <div style={{
                ...
                {/* Assigned Courses — only this scrolls */}
                <div style={{
                  border: "1px solid #e8e8e8", borderRadius: 8,
                  padding: "14px 16px",
                  background: "#fafafa",
                  ...
                  <style>{`
                    ...
                    .courses-scroll::-webkit-scrollbar-thumb { background: #ddd; border-radius: 99px; }
                    .courses-scroll::-webkit-scrollbar-thumb:hover { background: #bbb; }
                  `}</style>
                  <div className="courses-scroll" style={{
            ...
            <div style={{
              ...
              <button onClick={handleReject} style={{
                ...
                letterSpacing: ".05em", transition: "opacity .15s",
              }}
...
// ── Manage modal — for Approved / Suspended / Reactivated users
const ManageModal = ({ user, company, onClose, onSuspend, onBan, onReactivate }) => {
  const [action, setAction] = useState(null);
              ...
              <div style={{
                ...
                <div style={{
                  ...
                  <div style={{ width: "100%" }}>
                    ...
                    <div style={{
                      background: "#fff", borderRadius: 6, border: "none",
                      padding: "8px 10px", fontSize: 13, color: "#333",
                      boxShadow: "0 1px 4px rgba(0,0,0,.15)",
                    }}>
                      {user.username || "—"}
...
const ActionButton = ({ user, onView, onManage }) => {
  ...
  if (user.status === "Rejected") {
    return (
      <span style={{ fontSize: 12, color: "#bbb", fontStyle: "italic" }}>Rejected</span>
    );
  }
  // Approved / Suspended / Banned / Reactivated — show Manage
...
// ── Main ApprovalUsers ────────────────────────────────────────
const ApprovalUsers = ({ company, users: initialUsers, onBack }) => {
  const [users, setUsers]             = useState(initialUsers);
  const [viewUser, setViewUser]       = useState(null); // approval modal
  const [manageUser, setManageUser]   = useState(null); // manage modal
  ...
  const handleReject     = (user) => updateStatus(user, "Rejected");
  const handleSuspend    = (user, reason, duration) =>
    updateStatus(user, "Suspended", { suspendReason: reason, suspendDuration: duration });
  const handleBan        = (user, reason) =>
    updateStatus(user, "Banned", { banReason: reason });
  const handleReactivate = (user) =>
    updateStatus(user, "Reactivated", { suspendReason: null });
    ...
    <div style={{ padding: 24, minHeight: "100%", background: "#f4f4f4" }}>
      ...
      <div style={{
        ...
        <div style={{
          ...
          {getPages().map((p, i) =>
            p === "..." ? (
              <span key={`dot-${i}`} style={{ color: "#aaa", fontSize: 13, padding: "0 4px" }}>
                ...
              </span>
            ) : (
...
const st = {
  ...
  btnReject: {
    flex: 1, padding: "11px 0", background: "#fff", color: "#e74c3c",
    border: "1.5px solid #e74c3c", borderRadius: 8,
    fontWeight: 700, fontSize: 14, cursor: "pointer",
    fontFamily: "'Barlow', sans-serif", transition: "background .15s",
  },
  btnApprove: {
    flex: 1, padding: "11px 0", background: "#FF6B00", color: "#fff",
